...
void TraiterImage::Allouer()
{
    ...
    {
        m_ppImage[i]= new byte[ m_nWidth]; // BIEN RETENIR :
        m_normex[i]= new PIXEL[ m_nWidth];  // m_ppImage = double pointeur ,
        m_normey[i]= new PIXEL[ m_nWidth];  // m_ppImage[i] = pointeur sur le
        m_norme[i]= new PIXEL[ m_nWidth];   // vecteur = la ligne i.
...
// la  méthode seuillage par  intervalle
...
// la  méthode binarisation par intervalle
